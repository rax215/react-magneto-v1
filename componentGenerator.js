const generateComponent = (masterLayout,components) => {
  ...
  let componentOptions = {}, initialValues = {}, chartData = ''
  ...
  if(chartCmpList && chartCmpList.length>0) {
   chartCmpList.forEach(comp => {   
    ...
    import { ${[...new Set(chartCmpList.map((comp) => comp.attributes.type))].join(', ')} } from 'react-chartjs-2';\n`
  }

jsxCode = jsxCode + `import { ${[...new Set(materialComponents.map((comp) => comp.compName))].join(', ')} } from '@material-ui/core'
